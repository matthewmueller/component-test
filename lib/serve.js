...
module.exports = function(fn, options) {
  ...
  if (options.kill) {
    /**
     * Close the server when the document is loaded.
     */

    app.get('/close.gif', function(req, res, next) {
      server.close();
      process.exit();
    });
  }
 ...
 * `app.use()` so that it is mounted after the router
 * and we can add additional routes before it.
 */
